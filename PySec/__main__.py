...
class kms():
    ...
    def pin(self, rebase=False, kc=None, base=True):
        ...
        if self.trust and not rebase:
            ...
            messagebox.showerror(master=self.root, title="Error", message="For security reasons, you cannot export keys if you do not trust the current machine.")
        else:
            messagebox.showerror(master=self.root, title="Error", message="Hmm... We are not sure how to handle the current cryptographic situation...")
    ...
    def exportKeys(self, bases, tables, path):
